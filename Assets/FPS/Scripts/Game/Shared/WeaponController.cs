...
namespace Unity.FPS.Game
{
    ...
    public class WeaponController : MonoBehaviour
    {
        ...
        {
            // Toggle the configured weapon root if available. If not, fallback to enabling/disabling
            // renderers on the weapon so missing assignment won't leave the weapon visible.
            if (WeaponRoot != null)
            {
            ...
            {
                Debug.LogWarning($"WeaponController ({name}): WeaponRoot is null, falling back to toggling visuals.");
                // As a fallback, toggle the whole GameObject and force visuals state so visuals hide correctly
                gameObject.SetActive(show);
                ForceHideVisuals(show);
            }
            ...
            // Ensure visuals are in the right state even when WeaponRoot was present but some visuals aren't under it
            ForceHideVisuals(show);

            IsWeaponActive = show;
        }

        // Ensure all visual parts and particle systems are disabled when hiding the weapon. This is
        // used by ShowWeapon(false) to force-hide misconfigured prefabs that have visuals outside of
        // the WeaponRoot.
        void ForceHideVisuals(bool show)
        {
            // Renderers
