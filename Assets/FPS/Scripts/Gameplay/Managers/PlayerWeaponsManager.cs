    ...
    public class PlayerWeaponsManager : MonoBehaviour
    {
        ...
        public void SwitchToWeaponIndex(int newWeaponIndex, bool force = false)
        {
            if (force || (newWeaponIndex != ActiveWeaponIndex && newWeaponIndex >= 0))
            {
                ...
                Debug.Log($"SwitchToWeaponIndex called. CurrentIndex={ActiveWeaponIndex}, NewIndex={newWeaponIndex}, Force={force}");
                ...
                if (GetActiveWeapon() == null)
                {
                    ...
                    Debug.Log($"No active weapon previously; immediately switched to index {ActiveWeaponIndex} ({(newWeapon? newWeapon.name : "null")})");
                }
                // otherwise, remember we are putting down our current weapon for switching to the next one
                else
                {
                    m_WeaponSwitchState = WeaponSwitchState.PutDownPrevious;
                    Debug.Log($"Putting down previous weapon at index {ActiveWeaponIndex} to switch to {m_WeaponSwitchNewWeaponIndex}");
                }
            }
        }
        ...
        void UpdateWeaponSwitching()
        {
            ...
            if (switchingTimeFactor >= 1f)
            {
                if (m_WeaponSwitchState == WeaponSwitchState.PutDownPrevious)
                {
                    ...
                    if (oldWeapon != null)
                    {
                        Debug.Log($"Hiding old weapon at index {ActiveWeaponIndex} ({oldWeapon.name})");
                    ...
                    if (newWeapon)
                    {
                        m_TimeStartedWeaponSwitch = Time.time;
                        m_WeaponSwitchState = WeaponSwitchState.PutUpNew;
                        Debug.Log($"Starting put up for new weapon at index {ActiveWeaponIndex} ({newWeapon.name})");
                    }
                    else
                    {
